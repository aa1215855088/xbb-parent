...
<head>
    ...
    <link type="text/css" rel="stylesheet" href="${pageContext.request.contextPath}/css/a_002.css"
          source="widget">
    <link type="text/css" rel="stylesheet" href="${pageContext.request.contextPath}/css/a.css">
</head>
<body id="mainframe">
...
<div class="w w1 header clearfix">
    <div id="logo">
        <a href="/"><img src="${pageContext.request.contextPath}/images/XBB2.png" alt="新巴巴商城"></a> <a
...
<!-- main -->
<form action="${pageContext.request.contextPath}/order/createOrder" method="post">
    <div id="container">
        <div id="content" class="w">
                        ...
                        <div class="step-tit">
                            ...
                            <div class="extra-r">
                                <a href="javascript:;" class="ftx-05">新增收货地址</a> <input
                                    id="del_consignee_type" value="${user.username}-${user.addr}" type="hidden">
                            </div>
                        </div>
                        <div class="step-cont">
                                                ...
                                                <ul id="consignee-list">
                                                    <li
                                                        ...
                                                        <div class="consignee-item item-selected"
                                                            ...
                                                            <span limit="8"
                                                                  title="${user.realName}">${user.realName}</span><b></b>
                                                        </div>
                                                        <div class="addr-detail">
															<span title="${user.realName} " class="addr-name" limit="6"
                                                                  区 西三旗>${user.realName}
                                                            </span> <span class="addr-info"
                                                                          limit="45">${user.addr}</span> <span
                                                                class="addr-tel">${user.email}</span>
                                                        </div>
                        ...
                        <div id="shipAndSkuInfo">
                            <div id="payShipAndSkuInfo">
                                ...
                                <div class="step-tit">
                                    <h3>送货清单</h3>
                                    <div class="extra-r">
                                        <a href="http://localhost:1234/myCart" class="return-edit ftx-05">返回修改购物车</a>
                                    </div>
                                </div>
                                                                    ...
                                                                    <div class="goods-msg-gel">
                                                                        <div class="p-name">
                                                                            <a href="http://item.jd.com/1305606.html"
                                                                               target="_blank"> ${item.cartDTO.name}--${item.cartDTO.color}--${item.cartDTO.size} </a>
                                                                        </div>
                                                                        <div class="p-price">
                                                                            <!--增加预售金额显示 begin   预售分阶段支付类型（1：一阶梯全款支付；2：一阶梯定金支付(全款或定金可选)；3：三阶梯(仅定金)；4：三阶梯(全款或定金可选)） -->
                                                                            <strong class="jd-price">￥${item.cartDTO.price}</strong>
                                                                            <!--增加预售金额显示 end-->
                                                                            <span class="p-num"> x${item.amount} </span>
                                                                            <span
                                                                                    class="p-state"
                                                                                    skuid="1305606">${item.have?"有货":"没货"}</span>
                                                        ...
                                                        <ul class="mode-list">
                                                            <li>
                                                                <div class="fore1" id="payment_name_div">
                                                                    ...
                                                                    <input type="radio" value="1" name="paymentCash"
                                                                           checked="checked"/>现金
            ...
            <div class="order-summary">
                ...
                <div class="statistic fr">
                    <div class="list">
                        <span><em class="ftx-01">${cartInfo.productAmount}</em> 件商品，总商品金额：</span> <em
                            class="price" id="warePriceId" v="343.8">￥${cartInfo.productPrice}</em>
                    </div>
                    <div class="list">
                    ...
                    <div class="list">
                        <span>应付总额：</span> <em class="price" id="sumPayPriceId">￥${cartInfo.totalPrice}</em>
                    </div>
                </div>
                <div class="clr"></div>
            </div>
            ...
            <div class="trade-foot">
                <div class="consignee-foot">
                    <p>寄送至： 北京 海淀区 西三旗 ${user.addr} </p>
                    <p>收货人：${user.realName} ${user.email}</p>
                </div>
...
<script>
    function createOrder() {
        $.ajax({
            ...
            data: {
                paymentWay: $("input[name='paymentWay']").val(),
                paymentCash: $("input[name='paymentCash']").val(),
                note: $("input[name='note']").val()
            },
            success: function (data) {
                if (data.status == 1) {
                    window.location = "${pageContext.request.contextPath}/order/payOrder/" + data.data.id
                }
            }
        })
    }
